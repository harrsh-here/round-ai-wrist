...
import { Battery, Wifi, Bluetooth, Phone, Sun, Flashlight, X, Settings, Volume2, VolumeX, Vibrate } from 'lucide-react';
...
interface StatusBarProps {
  ...
  onSoundModeChange?: (mode: 'on' | 'vibrate' | 'off') => void;
}
...
const StatusBar = ({ 
  ...
  onBrightnessChange,
  soundMode: externalSoundMode = 'on',
  onSoundModeChange
}: StatusBarProps) => {
  ...
  const [brightness, setBrightness] = useState([externalBrightness || 75]);
  const [soundMode, setSoundMode] = useState<'on' | 'vibrate' | 'off'>(externalSoundMode);
  ...
  useEffect(() => {
    setSoundMode(externalSoundMode);
  }, [externalSoundMode]);
  ...
  const handleTouchEnd = (e: React.TouchEvent) => {
    ...
    if (Math.abs(dragDistance) > 50) {
      if (dragDistance > 0 && !showQuickSettings) {
        setShowQuickSettings(true);
      } else if (dragDistance < 0 && showQuickSettings) {
  ...
  const handleSoundModeToggle = () => {
    const modes: ('on' | 'vibrate' | 'off')[] = ['on', 'vibrate', 'off'];
  ...
  const getSoundIcon = () => {
    switch (soundMode) {
      case 'on': return Volume2;
      case 'vibrate': return Vibrate;
      case 'off': return VolumeX;
  ...
  if (showQuickSettings) {
    return (
      <>
        ...
        <div 
          className="fixed inset-0 z-45 bg-black transition-opacity duration-300 pointer-events-none"
          style={{ opacity: Math.max(0, (100 - brightness[0]) / 100 * 0.7) }}
        />
        
        <div 
          className="absolute inset-0 z-50 bg-black/95 backdrop-blur-md rounded-full"
          ...
          {!flashlight && (
            <div className="p-6 h-full overflow-y-auto watch-scroll">
              ...
              <div className="grid grid-cols-2 gap-3 mb-6">
                ...
                <div 
                  className="glass-bg p-4 rounded-xl cursor-pointer"
                  onClick={handleSoundModeToggle}
                >
                  <div className="flex items-center justify-between mb-2">
                    <div className="flex items-center space-x-2">
                      {React.createElement(getSoundIcon(), { 
                        size: 16, 
                        className: soundMode === 'off' ? 'text-gray-400' : 'text-primary' 
                      })}
